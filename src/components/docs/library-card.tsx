...
export function LibraryCard({ library, hideTitle = false }: LibraryCardProps) {
  ...
  return (
    <div className="border border-muted rounded-lg px-4 py-3">
      <div
        className={`flex items-center ${hideTitle ? 'justify-end' : 'justify-between'} mb-0.5`}
      >
        {!hideTitle && (
          <h4 className="font-medium text-[9px]">{library.name}</h4>
        )}
      ...
      <Button asChild variant="outline" size="sm">
        <a href={libraryUrl}>Learn More</a>
      </Button>
    </div>
  )
}
